...
Vel = $ ? $.Velocity : Velocity,
    ...
    (function ($) {
	  ...
	  function updateCounter(){
	    ...
	    var li_len = 0;
	    var str_count = 0;
    	var str_len = actualLength;

	    for (i = 0; i < str_len; i++) {
            var one_char = $(this).val().charAt(i);
            if (escape(one_char).length > 4) {
                str_count += 2; //한글이면 2를 더한다
            } else {
                str_count++; //한글아니면 1을 다한다
            }
		...
		actualLength  	  = str_count;
		var isValidLength     = parseInt(str_count) <= parseInt(maxLength);
	    ...
	    addInputStyle(isValidLength,actualLength, $(this));
	  ...
	  function addInputStyle(isValidLength,actualLength, $input){
	    var inputHasInvalidClass = $input.hasClass('invalid');
	    if (actualLength > 0 && isValidLength) {
